...
class OpenLegalAccountStatus:
    def __init__(self, request: pprb_request.SendRequest):
        self.request = request
        self.response: Optional[Response] = None

    # API_METHODS
    @allure.step('Выполнить запрос OpenLegalAccountStatusRq')
    def send_request(self, *args, **kwargs):
        self.response = self.request.post(
            '/v1/openLegalAccountStatus', *args, **kwargs
        )
    ...
    def verify_status_process(self, status_process):
        response_body = self.response.json()
        ...
        assert actual_status_process == status_process, (
            f'В теле ответа получен statusProcess = {actual_status_process}. '
            f'В теле ответа ожидался statusProcess = {status_process}.'
        )

    @allure.step(
        'Проверить, что поле response/statusList в теле ответа соответствует ожидаемому.'
    )
    def verify_status_list(self, **status_list):
        response_body = self.response.json()
        # actual_status_list = 'example'
        try:
            actual_status_list = response_body['response']['statusList']
        except KeyError:
            actual_status_list = None
        assert actual_status_list == status_list, (
            f'В теле ответа получен statusList = {actual_status_list}. '
            f'В теле ответа ожидался statusList = {status_list}.'
        )
